...
CREATE INDEX idx_trips_destination_point   ON trips USING GIST (destination_point);
...
CREATE INDEX idx_bookings_hold_expires     ON bookings (hold_expires_at) WHERE status = 'capacity_held';

-- Composite index for capacity check queries
CREATE INDEX idx_bookings_trip_active      ON bookings (trip_id, status)
